...
- Compression:
  ...
  - `deflateCopy` returns `Z_STREAM_ERROR` for a stream that has already started on IGZIP, leaving both streams untouched. ISA-L has emitted a header and still holds unflushed stream state, and that state cannot be duplicated: its level buffer holds pointers into its own allocation, so a copy of it would share the source's pending output. Flushing that pending output first is not an alternative either, since those bytes belong to the prefix the two streams share and `deflateCopy` has no way to return bytes to the caller. Copying a deflate stream before its first `deflate` call — the usual use, to compress several alternative tails after a common prefix, or to size the output — is unaffected on every path, as is `inflateCopy` at any point in a stream.
