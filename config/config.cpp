...
bool LoadConfigFile(std::string& file_content, const char* filePath) {
  ...
  configReader.GetValue("iaa_compress_percentage", iaa_compress_percentage, 100, 0);
  configReader.GetValue("iaa_decompress_percentage", iaa_decompress_percentage, 100, 0);
  configReader.GetValue("iaa_prepend_empty_block", iaa_prepend_empty_block, 1, 0);
