...
TEST_F(IAAWindowRejectionTest, UncompressIAAReportsOversizedWindow) {
  ...
  // The case the block above cannot tell apart from a no-op: starting from an
  // already-true flag, so that an implementation which clears the
  // out-parameter on every successful call -- rather than leaving an untouched
  // one alone -- would be caught rather than passing vacuously.
  input_len = static_cast<uint32_t>(narrow.size());
  output_len = static_cast<uint32_t>(output.size());
  end_of_stream = false;
  bool already_true = true;
  EXPECT_EQ(UncompressIAA(reinterpret_cast<uint8_t*>(&narrow[0]), &input_len,
                          output.data(), &output_len, qpl_path_software,
                          /*window_bits=*/-12, &end_of_stream,
                          /*detect_gzip_ext=*/false, &already_true),
            0);
  EXPECT_TRUE(already_true);
...
TEST_F(IAAWindowRejectionTest, RememberedRejectionWithNoFallbackDoesNotRefuse) {
...
#ifdef USE_QAT
  // Unlike IGZIP above, enabling QAT here does not stand the suppression
  // down: QAT's eligibility is a buffer/window check with no view of whether
  // a device is present, so treating it as a safe alternative could turn a
  // stream IAA can decode into a refusal on a host where QAT is compiled in
  // but not functional -- the reason inflate() does not count qat_available
  // among the terms that let it suppress IAA. With QAT the only other engine
  // enabled, the verdict therefore stays out of the decision and IAA gets
  // the same chance at the narrow stream as it would with no other engine at
  // all, which is exactly what the traffic split pinned at 100% IAA is here
  // to make unambiguous.
  ...
  EXPECT_EQ(InflateWholeStream(&to_qat, narrow, input, input_length),
            Z_STREAM_END);
  EXPECT_EQ(GetInflateExecutionPath(&to_qat), IAA);
